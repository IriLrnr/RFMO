...
InitialState = 0; % Start the voting from an open access equilibrium (i.e., overfished)

RunAnalyses = 0; % Start the voting from an unfished state (i.e., underfished)
if RunAnalyses ~= 0

    N = 3; % How many countries are there?
    ...
    eval(['save TEMP_' num2str(InitialState)])

else
    eval(['load TEMP_' num2str(InitialState)])
end
...
% When the historical state was exploratory , all nations take an equal
% proportion of the total harvesting effort , $H/n$. When the voting system is
% supermajority , the pivotal player will be the nation with the second highest
% environmental concern , i.e., the nation with $w_i = 0.8$, Nation 6. The
% voting dynamics will end at the collective harvesting effort H_bullet that is
% the collective harvesting effort that maximises Nation 6’s utility .
%
% Here , we will plot how Nation 6’s utility changes with the collective
% harvesting effort to check if the solution found by simulations ---
% H_bullet = 0.0616 and h_6 = 0.0088 --- is correct .
% parameters
% ---
% simulation result for the voting dynamical endpoint
H_bullet_sim = 0.0616;
% cost of harvesting
c = 0;
% the environmental concern of Nation 6
w6 = 0.8;
% number of grid points to plot
ngrid = 51;
% calculate how utility to Nation 6 varies with H
% ---
% the utility to Nation 6 is a function of collective harvesting effort
% Nation 6’s personal harvesting effort will be 1/7 th of the total
utility6_fnc = @(H) (1 - H)^ w6 * (( H /7) * (1 - c - H)) ^(1 - w6 );
% grid points for H in 0..1
HV = linspace (0 , 1, ngrid );
% calculate utility to Nation 6 for each H value
utility6V = zeros (1 , ngrid );
for idx = 1: ngrid
utility6V (1 , idx ) = utility6_fnc ( HV ( idx ));
end
% analytic solution
% ---
% in the document I sent , I showed the voting - dynamical endpoint found by
% solving the left root of a quadratic equation
H_bullet_quad = 1/2 * ( c + 2 * w6 + sqrt (4 * c * w6 ^2 + c ^2 - 8 * c * w6 + 2 * c + 1) - 3) / (w6 - 2) ;
% plot
% ---
% plot a vertical line for the simulation result
plot ([ H_bullet_sim , H_bullet_sim ] , [0 , 0.4])% , " color " , " blue ")
hold on ;
% plot a vertical line for result from the quadratic equation
plot ([ H_bullet_quad , H_bullet_quad ], [0 , 0.4])% , " color ", " red ")
% plot the curve of Nation 6’s utility versus the collective harvesting effort
plot (HV , utility6V)% , " color " , " black ") ;
% decorate the plot
title (" exploratory fishery , utility to pivotal nation (i. e., w = 0.8) ")
xlabel (" collective harvesting effort , H ")
ylabel (" utility to pivotal player , Nation 6")
legend (" optimum found by simulation " , " optimum found analytically ", " utility curve ")
hold off ;
